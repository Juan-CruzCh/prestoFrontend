...
import { crearTarifa, editarTarifasService, obtenerTarifasId } from "../service/tarifaService";
import { HttpStatus } from "../../../core/enum/httpSatatus";
import { AxiosError } from "axios";
import { AlertaError, exito } from "../../../core/utils/alertasUtils";


export const EditarTarifaPage = () => {
    const { id } = useParams()
    const { handleSubmit, control, watch, reset, setValue, formState: { errors } } = useForm<FormularioRango>({
        defaultValues: {
            nombre: "",
            rango1: 0,
            rango2: 0,
    ...
    const guardarTarifa = async (data: FormularioRango) => {
        ...
        try {
            if (id) {
                const response = await editarTarifasService(tarifa, id);
                if (response.status == HttpStatus.OK) {
                    exito("Tarifa editada")
                }
    ...
    const tarifas = async () => {
        ...
        try {
            if (id) {
                const response = await obtenerTarifasId(id)

                if (response) {
                    setValue("nombre", response.nombre)
                    setRangos(response.rango)
                }
            }
        } catch (error) {
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto min-h-screen bg-gray-50">
            <h1 className="text-2xl font-semibold mb-6 text-gray-800">Editar Tarifa</h1>
